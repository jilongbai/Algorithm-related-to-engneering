...
#define     N    33          //在0~32℃标定的温度点总数，33个
#define START_POINT_32      0.00
...
int main(void)
{
    ...
    while (1)
    {
		...
		/**********************************************************************************************************************************
		采样周期说明:
		(1)首先由于我们的温度最大变化率为6.5℃/s = 0.1℃/15.4ms
		(2)而我们要求的控制精度为0.1℃
		(3)可见对于AD的采样周期必须小于<15.4ms,才能保证准确度
		(4)而由奈奎斯特采样定律可知，采样率必须大于信号频率的两倍以上，即我们的采样周期必须<7.7ms
		(5)为了留有余量，我们采样频率至少应该为信号频率的4倍以上，则我们合理的采样周期应该为<3.85ms,我们可以设定为，3ms，2ms,1ms或者更小
		**********************************************************************************************************************************/
		
		/**********************************************************************************************************************************
		采样周期说明:
		(1)首先由于我们的温度最大变化率为6.5℃/s = 0.1℃/15.4ms = 0.05℃/7.7ms
		(2)而我们要求的控制精度为0.05℃
		(3)可见对于AD的采样周期必须小于<7.7ms,才能保证准确度
		(4)而由奈奎斯特采样定律可知，采样率必须大于信号频率的两倍以上，即我们的采样周期必须<3.85ms
		(5)为了留有余量，我们采样频率至少应该为信号频率的4倍以上，则我们合理的采样周期应该为<1.93ms,我们可以设定为,1ms或者更小
		**********************************************************************************************************************************/
				...
				**************************************************************************/
                ...
                case 2: //温度校验
						...
						//NOTE:ch9-ch15代表的是我们工装对各校准点105.00, 95.00, 90.00, 70.00, 60.00, 50.00, 30.00的对应数据
								  if((Col_DATA<(105000 + 200)) && (Col_DATA > (30000 - 200)))
								  {
									  if ((((CHANNLE_X - 1) / 8) == 0))
									  {
										  COR_T.u32_D[CHANNLE_X-1] = Col_DATA;
										  sEE_WriteBuffer((u8 *)COR_T.u8_D[4*(CHANNLE_X-1)], 0x02+32+4*(CHANNLE_X-1), 4); 
										  sEE_WaitEepromStandbyState();   
									  }
									  else 
									  {
										  GOZ_T.u32_D[CHANNLE_X-8-1] = Col_DATA;
										  sEE_WriteBuffer((u8 *)GOZ_T.u8_D[4*(CHANNLE_X-1)], 0x02+32+32+4*(CHANNLE_X-1), 4); 
										  sEE_WaitEepromStandbyState();     
									  }
...
//                                              {
//                                                  End = i;  //在触摸屏那边必须保证7位ASCII字符的长度，才会在触摸屏向MCU发送数据时才会以0XFF作为数据结束符
//                                                  break;   
//                                              }
                                                ...
                                                case 2:  //温度校验温度范围在[29.800, 105.200]范围内的时候，进行校准
														 ...
														 //NOTE:ch9-ch15代表的是我们工装对各校准点105.00, 95.00, 90.00, 70.00, 60.00, 50.00, 30.00的对应数据
                                                              if((Col_DATA<(105000 + 200)) && (Col_DATA > (30000 - 200)))
                                                              {
																  if ((((Set_ChX - 1) / 8) == 0))
																  {
																	  COR_T.u32_D[Set_ChX-1] = Col_DATA;
																	  sEE_WriteBuffer((u8 *)COR_T.u8_D[4*(Set_ChX-1)], 0x02+32+4*(Set_ChX-1), 4); 
																	  sEE_WaitEepromStandbyState();   
																  }
																  else 
																  {
																	  GOZ_T.u32_D[Set_ChX-8-1] = Col_DATA;
																	  sEE_WriteBuffer((u8 *)GOZ_T.u8_D[4*(Set_ChX-1)], 0x02+32+32+4*(Set_ChX-1), 4); 
																	  sEE_WaitEepromStandbyState();     
																  }
...
float Calculate_Temp_SZ(u16 AD_Res , float UP_Res , float V_ref )
{
    ...
    if(V_Res<2)
    {
		...
		//温度范围检查
        if(Res > 36668.6695) //32度以下用另外一个表查询
        {
					T = (Successive_Linear_Interpolation_Temp_Correction(Res))*10;
...
//当温度<32℃时候，需要通过另外一个表格进行查询，同样进行线性插值
...
输出参数：校准后的温度值
NOTE:此处采用点斜式计算y = y0 + (y1 - y0)(x - x0)/(x1 - x0)；线性方程
****************************************************************************************************************************************/
...
{
	//const float interval = 1.00;       //标定数据，温度间隔为1℃
	return (y0 + (y1-y0)*(x-x0)/(x1-x0));
}

/***************************************************************************************************************************************************************************
逐次线性插值法(抛物线插值)具体步骤如下：
已知:3点(x0, y0)、(x1, y1)、(x2, y2)及插值点的x,求对应的y值
用直线点斜式公式：

第1步：过点(x0, y0)和(x1, y1)作直线L01,即：

					L01 = y0 + (y1 - y0)*(x - x0) / (x1 - x0);                                                                            (2 - 1)
					
第2步：过点(x0, y0)和(x2, y2)作直线L02,即：

					L02 = y0 + (y2 - y0)*(x - x0) / (x2 - x0);                                                                            (2 - 2)
					
第3步：将(x1, L01)和(x2, L02)也理解为"点",则过这两点做"直线"，记之为L012，即：

					L012 = L01 + (L02 - L01)*(x - x1) / (x2 - x1);                                                                        (2 - 3)
					
第4步：把式(2 - 1)和式(2 - 2)代入式(2 - 3)：

L012 = y0 + [(y1 - y0)*(x - x0) / (x1 - x0)] +  {([(y2 - y0)/(x2 - x0)]-[(y1 - y0)/(x1 - x0)]) / (x2 - x1)} *(x - x0)*(x - x1);          (2 - 4)             
					
                                                                                       
***************************************************************************************************************************************************************************/
//当温度<32℃时候，需要通过另外一个表格进行查询，进行抛物线插值，实测与线性插值差别不是很大，说明用线性或者抛物线逼近其距离真实值差距不是很大，同时
//说明传感器在低于32度的时候，不成线性也不成二次多项式型
float Successive_Linear_Interpolation_Temp_Correction(float ResVal)
{
	u8 i;
	float x0, y0, x1, y1;
	const float interval = 1.00; 
	float L01, L02, L012;
	
	if (ResVal > RT_32[0])     //如果采样结果过低，则返回起点温度,即如果温度低于0℃，则记之为0℃
	{
		return (START_POINT_32);
	}
	
	for (i=1; i<N; i++)            //由于抛物线插值需要使用3个连续的标定节点，故在判断插值点ResVal所在区间的时候，应该从i=1开始搜索
	{
		if (ResVal > RT_32[i+1])     //举例，完成x所在区间的判断
			break;
	}
	x0 = RT_32[i-1]; 
	y0 = interval*(i-1) + START_POINT_32;
	x1 = RT_32[i];    
	y1 = interval*i + START_POINT_32;
	L01 = Linear_Interpolation(x0, y0, x1, y1, ResVal);     //过i-1节点和i节点作直线L01
	
	x1 = RT_32[i+1];    
	y1 = interval*(i+1) + START_POINT_32;
	L02 = Linear_Interpolation(x0, y0, x1, y1, ResVal);     //过i-1节点和i+1节点作直线L02
	...
	L012 = Linear_Interpolation(x0, y0, x1, y1, ResVal);    //过(RT_32[i], L01)节点和(RT_32[i+1], L02)节点作"直线"L012,完成抛物线插值
